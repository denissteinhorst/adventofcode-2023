...
export default function (): number {
  ...
  const matchingSymbols: { possibilitys: string[], coordinates: number[][] } = {
    possibilitys: data.input.match(/[\*]/g) || [],
    coordinates: []
  }
  ...
  matchingSymbols.coordinates.forEach((coordinate: number[]) => {
    ...
    if(extractAdjecentNumbers?.length === 2) {
      const [firstNumber, secondNumber] = extractAdjecentNumbers;
      const multiplier = parseInt(firstNumber) * parseInt(secondNumber);
      result += multiplier
    }
  });
